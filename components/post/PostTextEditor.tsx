...
const PostTextEditor = ({user, groupId, username}: PostTextEditorProps) => {
  ...
  const {useAddMutation, useAiGenerate} = usePost()
  const {mutateAsync, isPending} = useAddMutation()
  const {mutateAsync: mutateGenerateAiPost, isPending: isGenerating} = useAiGenerate()

  const editor = useCustomEditor({placeholder: "What's on your mind ... | prompt "})
  ...
  const handleAiPost = async () => {
    try {
      ...
      await mutateGenerateAiPost(input, {
        onSuccess: (res) => {
          setError(null)
          editor?.commands.clearContent()
  ...
  return (
    <div
      className='bg-background p-4 rounded-lg shadow-sm border flex flex-col overflow-hidden h-fit max-h-[500px] gap-y-2'>
      <div className='flex items-start gap-4'>
        ...
        <div className='w-full'>
          <div className='flex items-start justify-between gap-1'>
            ...
            />
            <Button onClick={handleAiPost} className={`flex items-center gap-1 text-sm ${isGenerating ? 'opacity-90' : ''}`} size='sm' variant='outline'>
              <Sparkles className='w-4'/>
              {isGenerating ? <Loading/> : <span className='hidden sm:inline-block text-sm'>
								Generate
							</span>}
          ...
          {error?.prompt?.[0] && <InputError error={error?.prompt?.[0]!}/>}
